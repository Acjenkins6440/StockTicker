...
const StockContainer = ({ endPoint, symbol }) => {
  ...
  const maxP = () => Math.max(...dataSet.map(d => d.p));
  const minP = () => Math.min(...dataSet.map(d => d.p));
 ...
 const yScale = d3
  ...
  useEffect(() => {
   ...
   const line = d3
     .line()
     .x(d => xScale(d.t)) // set the x values for the line generator
     .y(d => yScale(d.p)) // set the y values for the line generator
